...
export default function allGroups() {
    ...
    const handleConfirmDeactivation = async () => {
        if (selectedGroupId) {
            await deleter(`/groups/${selectedGroupId}/`);
            setShowModal(false);
            mutate(`/groups?page=${currentPage}`);

            toast.success(`User Group has been deleted!`, {
                position: "top-right",
                theme: "dark",
            });
        }
                            ...
                            <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                                ...
                                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                    {groups?.results.map((group) => (
                                        <tr key={group.id}>
                                            <td className="px-6 py-2.5 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">{group.name}</td>
                                            <td className="px-6 py-2.5 whitespace-nowrap text-end text-sm font-medium">
                                                <Link href={`/admin/user-groups/edit/${group.id}`}>
                                                ...
                                                <button onClick={() => handleDeactivateClick(group.id)} type="button" className="inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent text-blue-600 hover:text-blue-800 disabled:opacity-50 disabled:pointer-events-none dark:text-blue-500 dark:hover:text-blue-400 dark:focus:outline-none dark:focus:ring-1 dark:focus:ring-gray-600">
                                                    Delete
                                                </button>
                                            </td>
                                        </tr>
